...
## 📊 5. 신규 UI 및 콤보 스킬, 어트리뷰트 적용 가이드

### 🪟 5.1 대미지 텍스트 팝업 (Damage Text)
몬스터 타격 시 대미지 수치를 화면에 띄웁니다.
1. `LostArkDamageTextActor`를 부모로 하는 블루프린트(`BP_DamageTextActor` 등)를 생성합니다. 위젯 부착용 액터입니다.
2. 캐릭터 블루프린트의 디테일 패널에서 **Character | UI** 탭에 있는 `Damage Text Class`에 해당 블루프린트를 할당합니다.
3. 타격 판정 성공 시 인터페이스(`ShowDamageText`)를 통해 자동으로 화면에 팝업됩니다.

### ⚔️ 5.2 콤보 스킬 연계 (Combo Attack)
여러 번 키를 눌러 연속 공격(예: 평타 3연타)을 구현합니다.
1. `LostArkComboSkillAbility`를 부모로 하는 스킬 블루프린트를 생성합니다.
2. 캐릭터의 기본 평타 콤보용이라면 캐릭터 블루프린트의 `Combo Attack Ability Class`에 할당합니다.
3. 로직 내부에서 콤보 카운트를 증가시키고, 다음 연계 몽타주를 차례대로 재생하여 다단 콤보를 손쉽게 구축할 수 있습니다.

### 💖 5.3 어트리뷰트 세트 (Attribute Set) 및 아이덴티티(Identity)
체력, 마나, 공격력, 이동 속도 등의 기초 스탯과 **아이덴티티 게이지**를 관리합니다.
1. 코어 스탯들은 이미 `LostArkAttributeSet` 클래스 내부에 선언되어 있습니다.
2. **아이덴티티 시스템**: 스킬 타격 성공 시 자동으로 아이덴티티 게이지(`IdentityGauge`)가 차오르며, 100% 도달 시 `State.IdentityBurst` 태그가 부여되며 폭주 상태에 진입합니다. 폭주 상태에서는 게이지가 서서히 소모됩니다. (HUD 위젯에 자동으로 게이지 바가 연동됩니다.)
3. **Gameplay Effect(GE)** 를 생성한 뒤 `Modifiers` 배열에서 제어할 속성(예: `LostArkAttributeSet.Health`)을 선택하고 Add, Multiply 등의 연산을 설정하여 스탯을 조작할 수 있습니다.

### ⏱️ 5.4 스킬 슬롯 UI 및 쿨타임 연동 (Skill Slot UI)
스킬 아이콘과 쿨타임(재사용 대기시간)을 시각적으로 표시합니다.
1. `LostArkSkillSlotWidget`을 상속받는 위젯 블루프린트(`WBP_SkillSlot` 등)를 생성합니다.
2. 해당 위젯 블루프린트의 디테일 패널이나 초기화 로직에서 모니터링할 **`Cooldown Tag`** (예: `Cooldown.Skill.Q`)를 지정합니다.
3. 플레이어가 스킬을 사용하여 해당 쿨타임 태그가 ASC에 부여되면, 위젯이 이를 즉각적으로 감지하여 남은 시간 텍스트와 UI 게이지 애니메이션(`OnCooldownStarted`)을 자동으로 처리합니다.
