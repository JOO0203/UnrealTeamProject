...
void ALostArkCharacter::PossessedBy(AController* NewController)
{
	...
	if (AbilitySystemComponent)
	{
		...
		if (AbilitySystemComponent)
		{
			...
			AbilitySystemComponent->GenericGameplayEventCallbacks.FindOrAdd(FGameplayTag::RequestGameplayTag(FName("Event.Player.CounterSuccess"))).AddUObject(this, &ALostArkCharacter::HandleCounterSuccessEvent);
		}
	}
}
...
{
	// 보스 측에서 카운터 적중에 성공했다는 이벤트를 보내주면 UI 연출 발동
	OnCounterSuccess_UI();
}
